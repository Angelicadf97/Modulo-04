<%@ page language="java" contentType="text/html" pageEncoding="UTF-8"
	import="model.Voo" import="model.Aeroporto" import="model.Local"
		...
		<main class="container conteudo my-4 p-4">
				...
				<div class="row mb-3">
					<div class="form-group col">
						...
						<select id="idAP" name="idAP" class="form-select">
							...
							%>
							<option value="<%=ap.getId()%>"><%=ap.getNome() + " - " + ap.getLocal().getCidade() + "/" + ap.getLocal().getUf()%></option>
							<%
							}
							%>
						</select>
					...
					<div class="form-group col">
						...
						<select id="idAD" name="idAD" class="form-select">
							<option value="DEFAULT">Escolha um aeroporto</option>
							<%
							for (Aeroporto ad : aeroportos) {
							%>
							<option value="<%=ad.getId()%>"><%=ad.getNome() + " - " + ad.getLocal().getCidade() + "/" + ad.getLocal().getUf()%></option>
							<%
							}
							%>
						</select>
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						<td><%=v.getId()%></td>
						<td><%=v.getAeroPartida().getLocal().getCidade() + "/" + v.getAeroPartida().getLocal().getUf()%></td>
						<td><%=v.getAeroDestino().getLocal().getCidade() + "/" + v.getAeroDestino().getLocal().getUf()%></td>
						<td><%=v.getData()%></td>
						<td><%=v.getHorario()%></td>
						<td>R$ <%=v.getPrecoBase()%></td>
						<td><%=v.getAssentos()%></td>
						<td><%=v.getAeroPartida().getNome()%></td>
						<td><%=v.getAeroDestino().getNome()%></td>
						<td class="d-flex justify-content-start align-items-center">
							<div>
								<!-- Button trigger modal -->
								<button class="btn" type="button" data-bs-toggle="modal"
									data-bs-target="#staticBackdrop<%=v.getId()%>">
									<i class="gg-pen"></i>
								</button>

								<!-- Modal -->
								<div class="modal fade" id="staticBackdrop<%=v.getId()%>"
							...
							</div> <a href="./voo-delet?id=<%=v.getId()%>"
